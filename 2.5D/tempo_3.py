...
# Variables de suivi pour les sauvegardes multiples
best_bal_acc = 0.0
best_val_loss = float('inf')

# Noms des fichiers
FILE_BEST_BAL = "best_model_bal_acc.pth"
FILE_BEST_LOSS = "best_model_loss.pth"
FILE_FINAL = "model_final.pth"
...
for epoch in range(num_epochs):
    ...
    # 1. Meilleure Balanced Accuracy (Critère principal)
    if val_bal > best_bal_acc:
        best_bal_acc = val_bal
        torch.save(model.state_dict(), FILE_BEST_BAL)
        print("  ★ Saved: Best Balanced Accuracy")

    # 2. Meilleure Validation Loss (Pour éviter l'overfitting)
    if val_loss < best_val_loss:
        best_val_loss = val_loss
        torch.save(model.state_dict(), FILE_BEST_LOSS)
        print("  ☆ Saved: Best Validation Loss")

# 3. Modèle Final (Dernière époque)
torch.save(model.state_dict(), FILE_FINAL)
print(f"  -> Saved: Final Model ({FILE_FINAL})")
...
def plot_curves():
    ...
    # plt.ylim(0, 1.0) # Désactivé pour le zoom auto
    plt.title('Accuracy')
    plt.xlabel('Epochs')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig('training_curves_multi_save.png', dpi=300)
    print("\nGraphiques sauvegardés : training_curves_multi_save.png")

plot_curves()

# ==========================================
# 7. TEST COMPARATIF DES 3 MODÈLES
# ==========================================
print("\n" + "="*40)
print(" COMPARATIF FINAL SUR LE TEST SET")
print("="*40)

models_to_test = [
    ("Best Balanced Acc", FILE_BEST_BAL),
    ("Best Val Loss", FILE_BEST_LOSS),
    ("Final Model", FILE_FINAL)
]

for name, path in models_to_test:
    if os.path.exists(path):
        model.load_state_dict(torch.load(path))
        t_loss, t_acc, t_bal = evaluate(model, test_loader, device, criterion)
        print(f"[{name:^18}] Acc: {t_acc:.4f} | Bal Acc: {t_bal:.4f} | Loss: {t_loss:.4f}")
    else:
        print(f"[{name:^18}] Fichier introuvable ({path})")
print("="*40)